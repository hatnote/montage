...
def make_entry(edict):
    ...
    if edict.get('oi_archive_name'):
        # The file has multiple versions — capture the original (first) upload info (#155)
        flags = {
            'reupload': True,
        ...
        # Preserve original author from the first-ever revision if available
        if edict.get('orig_upload_user_text'):
            flags['orig_upload_user_text'] = edict['orig_upload_user_text']
            flags['orig_upload_user_id'] = edict.get('orig_upload_user_id')
            flags['orig_upload_date'] = wpts2dt(edict['orig_upload_date']) if edict.get('orig_upload_date') else None
