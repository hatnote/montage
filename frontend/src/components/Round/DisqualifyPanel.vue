    ...
    <div class="disqualify-panel">
        <div class="diqualify-panel-controls">
            <div class="disqualify-grid-size-controls">
                <cdx-button :action="gridSize === 3 ? 'progressive' : ''" weight="quite" @click="setGridSize(3)">
                    <image-size-select-actual class="icon-small" />
                    {{ $t('montage-vote-grid-size-large') }}
                </cdx-button>
                <cdx-button :action="gridSize===2 ? 'progressive' : ''" weight="quite" @click="setGridSize(2)">
                    <image-size-select-large class="icon-small" />
                    {{ $t('montage-vote-grid-size-medium') }}
                </cdx-button>
                <cdx-button :action="gridSize===1 ? 'progressive' : ''" weight="quite" @click="setGridSize(1)">
        ...
        <div class="disqualify-grid" :class="'grid-size-' + gridSize">
            ...
            <div v-for="entry in entries" :key="entry.id" class="disqualify-entry" :class="{ 'disqualify-entry--disqualified': entry.dq_user_id }">
                ...
                <div class="disqualify-entry-info">
                    ...
                    <p v-if="entry.dq_reason" class="disqualify-entry-reason">
                     ...
                     <div class="disqualify-entry-actions">
                        <a :href="'https://commons.wikimedia.org/wiki/File:' + encodeURIComponent(entry.name)" target="_blank" rel="noopener">
                            <cdx-button size="medium">
                                <link-icon class="icon-small" />
                                {{ $t('montage-view-on-commons') }}
                            </cdx-button>
                        </a>
                        <cdx-button v-if="canModify && !entry.dq_user_id" action="destructive" @click="$emit('disqualify', entry)" >
                            <cancel-icon class="icon-small" />
                            {{ $t('montage-disqualify-action') }}
                        </cdx-button>
                        <cdx-button v-if="canModify && entry.dq_user_id" action="progressive" @click="$emit('requalify', entry)" >
