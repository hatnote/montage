const Service = ($http, $q, $window) => {
  ...
  const admin = {
    ...
    addCoordinator: (id, username) => $http.post(base + '/campaign/' + id + '/add_coordinator', { username }),
    removeCoordinator: (id, username) => $http.post(base + '/campaign/' + id + '/remove_coordinator', { username }),
