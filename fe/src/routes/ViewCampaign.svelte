<MontageAPI api='/v1/admin/campaign/{params.id}' let:data={data}>
  <Paper>
    <Content>
      ...
      <Subtitle>{data.open_date} - {data.close_date}</Subtitle>
      <Set chips={data.coordinators} let:chip>
        <Chip><Text>{chip.username}</Text><Icon class="material-icons"  tabindex="0">person</Icon></Chip>
      </Set>
